...
print(get_index_first_non_zero_value_from_column(M,1,1))
...
def get_index_first_non_zero_value_from_row(M,row,augmented=False):
    ...
    for i,val in enumerate(row_array):
        if not np.isclose(val,0,atol=1e-08):  #if we found the very first non_zero element then we just return the row index
            return i
    return -1
print(get_index_first_non_zero_value_from_row(M,0,augmented=False))
...
def row_echelon_form(A, B):
    ...
    for row in range(num_rows):
        ...
        if np.isclose(pivot_candidate, 0) == True:
            ...
            if first_non_zero_value_below_pivot_candidate == -1:  #we must check this , cause if no non_zero numbers are found then there is no point in swapping
                #cause further swapping might cause disorder of row echelon form
                continue
        ...
        for j in range(row + 1, num_rows):
            ...
            M[j] = M[j] - value_below_pivot * M[row] #as we have to change the very first value in the rows below pivot to zero we are using value_below_pivot
    return M  #now the first column is done

#back substitution method

def back_substitution(M):
    # Making a copy of the input matrix to avoid modifying the original
    M = M.copy()
    num_rows = M.shape[0]   #lenghth of the row in the coefficient matrix

    #LOGIC

    # Iterate from bottom to top
    for row in reversed(range(num_rows)):
        substitution_row = M[row]   #we use this row to subtract other row elements to make their elements 0s.
        ...
        index = get_index_first_non_zero_value_from_row(M,row,augmented=True)  #finding the index of the first non_zero element in the substitution row
...
row_M =row_echelon_form(A, B)
print(row_M)
final = back_substitution(row_M)
print(final)
